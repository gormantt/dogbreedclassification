...
#function based on udacity script solution fintune_a_cnn_solution.py
def test(model, test_loader, criterion, device):
    ...
    print(
        "\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n".format(
            total_loss, running_corrects, len(test_loader.dataset), 100*total_acc
        )
    )
    return
...
def main():
    ...
    torch.save(model, "/opt/ml/model/dogbreed_resnet50.pt")
